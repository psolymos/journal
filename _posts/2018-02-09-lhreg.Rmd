...
It took a few years, and the [most positive peer-review experience](https://twitter.com/psolymos/status/903634823906033664),
but the paper is now early view in [*Ecography*](http://onlinelibrary.wiley.com/doi/10.1111/ecog.03415/abstract). This post is a quick tutorial of the [**lhreg**](https://github.com/borealbirds/lhreg#readme) R package that makes the whole analysis reproducible, and provides some functions for similar PGLMM models.
...
Here is the [html](https://borealbirds.github.io/lhreg/).
...
and fits the a Multivariate Normal model to the observed `Y` vector
...
The function is pretty barebones (i.e. no formula interface,
the design matrix `X` needs to be properly specified through
e.g. `model.matrix()`). The `lambda` argument
is a nonnegative number modifying the strength of phylogenetic effects. 
`lambda = 0` is equivalent to `lm` with
`weights = 1/SE`, `lambda = 1` implies Brownian motion evolution,
...
Besides standard AIC based techniques, the vignette describes a few 
techniques which are pretty nice to have in
a multivariate setting (i.e. profile likelihood, parametric bootstrap)
to support hypothesis testing and model selection.
...
The conditional distribution is described in the paper which
made this exercise very straightforward.
Maybe it is just ignorance, but I couldn't find another paper
that would have described it very usefully.
...
The following figure showing the two input data vectors side-by-side:

![](https://github.com/borealbirds/lhreg/raw/master/tree.png)

I realize this is not a very detailed post, so feel free to ask question in the discussion!
