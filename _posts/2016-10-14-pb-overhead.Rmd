...
As a testament to mey obsession of progress bars in R, here is 
a quick investigation about the overhead cost of 
drawing a progress bar during computations in R.
I compares several approaches including
my **pbapply** and Hadley Wickham's **plyr**.

Now compare the good old `lapply` function from base R,
...
The function `f` runs all four variants. The expected run time
is `n * s` (number of iterations x sleep duration), 
therefore we can calculate the overhead from the
return objects as elapsed minus expected. Let's see it
for a variaty of `n` values and replicated `B` times
...
There are OS specific differences it seems.

```{r dev='png',fig.width=7,fig.height=3}
png("pb-overhead.png", width = 700, height = 450)
op <- par(mfrow=c(1, 2))
